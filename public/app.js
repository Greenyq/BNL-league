...
// Calculate points for a single match based on MMR difference
const calculateMatchPoints = (match) => {
    const mmrDiff = match.mmrDiff || 0;

    if (match.result === 'win') {
        if (mmrDiff >= 20) return 30;      // vs stronger (+20 MMR)
...
// Get the index of the final match in a streak
const getStreakEndIndex = (recentMatches, streakLength, result = 'win') => {
    let currentStreak = 0;
    for (let i = recentMatches.length - 1; i >= 0; i--) {
        if (recentMatches[i].result === result) {
            currentStreak++;
            if (currentStreak === streakLength) return i;
        } else {
            currentStreak = 0;
        }
    }
                ...
                <div style={{ marginBottom: '40px', padding: '25px', background: '#2a2a2a', borderRadius: '15px', border: '2px solid #4caf50' }}>
                    ...
                    <div style={{ fontSize: '1.1em', lineHeight: '1.8', color: '#e0e0e0' }}>
                        <p style={{ marginBottom: '15px' }}>
                            <strong>Breaking New Limits</strong> — это профессиональная лига Warcraft 3: Reforged, где лучшие игроки со всего региона соревнуются за звание чемпиона.
                        </p>
                        <p style={{ marginBottom: '15px' }}>
                            Лига состоит из <strong>3 этапов</strong>. На каждом этапе игроки зарабатывают <strong>очки</strong>, которые определяют их продвижение. Очки рассчитываются автоматически на основе ваших побед в W3Champions и выполненных достижений.
                        </p>
                        <div style={{
                            background: '#1a1a1a',
                            padding: '15px',
                ...
                <div style={{ marginBottom: '40px' }}>
                    ...
                    <div style={{ fontSize: '1.1em', lineHeight: '1.8', color: '#e0e0e0' }}>
                        ...
                        <ul style={{ marginLeft: '20px', marginBottom: '15px' }}>
                            ...
                            <li style={{ marginBottom: '10px' }}>
                                <strong style={{ color: '#c9a961' }}>👨‍🏫 Тренер</strong> — помогает команде с советами по стратегии, анализирует игры противников и помогает игрокам совершенствоваться
                            </li>
                        </ul>
                        <p style={{ marginBottom: '15px' }}>
