...
function DuelsTab({ players, showMsg, onRefresh }) {
    ...
    const loadDuels = React.useCallback(() => Promise.all([apiFetch('/api/duels'), apiFetch('/api/duels/stage2?revealNames=1')]).then(([d, stage2]) => {
        ...
        setEncounters((stage2?.participants || []).filter(p => p.encounterStatus === 'pending'));
    }), []);
    ...
    return <div>
        ...
        {!!encounters.length && <div className="card-elevated" style={{ padding: 18, marginBottom: 18 }}><h4 style={{ marginTop: 0 }}>{tr('Активные DnD-события', 'Active DnD encounters')}</h4>{encounters.map(encounter => <div key={encounter.id} style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 10, padding: '10px 0', borderTop: '1px solid rgba(212,175,55,.2)' }}><strong>{encounter.name}</strong><span>— {encounter.encounterType === 'dragon' ? '🐉 Dragon Player' : '🏰 Dungeon Boss'}: {encounter.encounterOpponentName}</span><button className="btn btn-primary" type="button" onClick={() => resolveEncounter(encounter, true)}>{tr('Игрок победил', 'Player won')}</button><button className="btn btn-secondary" type="button" onClick={() => resolveEncounter(encounter, false)}>{tr('Игрок проиграл', 'Player lost')}</button></div>)}</div>}
