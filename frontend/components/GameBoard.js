...
function PlayerPanel({ self, king, guardian, snapshot, onSafe, onMystery, onFind, onUseRelic, pathError, relicError, relicBusy }) {
    ...
    return <aside className="dnd-player-panel">
        ...
        <section className={`dnd-map-assignment${self?.assignedMapPreviewUrl ? ' has-preview' : ''}`}>{self?.assignedMapPreviewUrl && <img src={self.assignedMapPreviewUrl} alt={self.assignedMapTitle || self.assignedMapBiome || 'Assigned map'} />}<div><label>MAP BIOME</label><strong>{self?.assignedMapBiome || 'Not assigned'}</strong><small>{self?.assignedMapSeason || 'Active season'}{self?.assignedMapTitle ? ` · ${self.assignedMapTitle}` : ''}</small></div></section>
