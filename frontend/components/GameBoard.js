...
export function GameBoard({ participants = [], duels = [], viewer = {}, revealNames = false, onRevealNames }) {
    ...
    return <div className="dnd-board-shell">
        ...
        <div className={`dnd-campaign-layout${snapshot.matches('defeat') ? ' is-defeat' : ''}${!self ? ' is-spectator' : ''}`}><CampaignMap participants={participants} snapshot={snapshot} focus={focus} />{self ? <PlayerPanel self={self} king={king} guardian={guardianName} snapshot={snapshot} onSafe={() => choosePath('safe')} onMystery={() => choosePath('mystery')} onFind={findSelf} onUseRelic={useRelic} pathError={pathError} relicError={relicError} relicBusy={relicBusy} /> : <aside className="dnd-player-panel dnd-spectator-panel"><small>PLAYER CAMPAIGN</small><h3>{!viewer?.isAuthenticated ? 'Войдите в аккаунт' : !viewer?.hasLinkedPlayer ? 'Привяжите BattleTag' : 'Игрок не добавлен в Сезон 3'}</h3><p>{!viewer?.isAuthenticated ? 'После входа здесь появятся ваша статистика, следующий бой и выбор дороги.' : !viewer?.hasLinkedPlayer ? 'Откройте профиль и привяжите BattleTag к аккаунту.' : 'Обратитесь к администратору, чтобы добавить игрока в текущий сезон.'}</p></aside>}</div>
